...
WORKDIR /src
COPY ["AWSLambdaContainerImage.csproj", "AWSLambdaContainerImage/"]
RUN dotnet restore "AWSLambdaContainerImage/AWSLambdaContainerImage.csproj"

WORKDIR "/src/AWSLambdaContainerImage"
COPY . .
RUN dotnet build "AWSLambdaContainerImage.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AWSLambdaContainerImage.csproj" \
...
Execute unit tests
```
cd "AWSLambdaContainerImage/test/AWSLambdaContainerImage.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "AWSLambdaContainerImage/src/AWSLambdaContainerImage"
